...
def Register():
    ...
    if request.method == "GET":
        ...
        while os.path.exists(picture_path):
            hash_object = md5(
                str(generate_random_number()).encode()).hexdigest()
            picture_path = "static/media/pictures/{}.png".format(hash_object)
...
def login():
    ...
    if request.method == "GET":
        ...
        user_data = db.execute(
            "SELECT * FROM users WHERE id=?", data[0]["user_id"])
...
def patients():
    patients = db.execute(
        "SELECT user_patients.id,name,email,phone_number,city,register_date,picture_path FROM user_patients INNER JOIN patients ON patients.id=user_patients.patient_id WHERE user_patients.user_id=? ORDER BY name", session["user_id"])
    for patient in patients:
        ...
        if len(next_appointment) == 0:
            ...
            patient["next_appointment"] = next_appointment[0]["date"] + \
                " " + next_appointment[0]["time"]

        if len(last_appointment) == 0:
            patient["last_appointment"] = "-"
        else:
            patient["last_appointment"] = last_appointment[0]["date"] + \
                " " + last_appointment[0]["time"]
...
def delete_patient(id):
    db.execute("DELETE FROM appointments WHERE doctor_id=? AND patient_id=?",
               session["user_id"], id)
...
def add_patient():
    if request.method == "GET":
        ...
        db.execute("INSERT INTO user_patients (user_id, patient_id) VALUES (?,(SELECT id FROM patients WHERE email=?))",
...
def add_appointment():
    patients = db.execute(
        "SELECT user_patients.id,name,picture_path FROM user_patients INNER JOIN patients ON patients.id=user_patients.patient_id WHERE user_patients.user_id=?", session["user_id"])
    if request.method == "GET":
        ...
        appointments = db.execute(
            "SELECT time, date from appointments WHERE link_id=? AND date=?", link_id, date)

        for appointment in appointments:
            if abs(minutes_between(date, time, appointment["date"], appointment["time"])) < 30:
                error = "You can't add an appointment on this day at " + time + \
                    ". Because There's another appointment at " + \
                        appointment["time"] + "."
...
def change_password():
    if request.method == "GET":
        ...
        data = db.execute(
            "SELECT password_hash FROM user_login WHERE user_id = ?", session["user_id"])
        ...
        password_hash = generate_password_hash(password)
        db.execute("UPDATE user_login SET password_hash=? WHERE user_id=?",
...
def edit_info():
    ...
    data["username"] = db.execute(
        "SELECT username FROM user_login WHERE user_id=?", session["user_id"])[0]["username"]
    if request.method == "GET":
        ...
        if username:
            ...
            db.execute("UPDATE user_login SET username=? WHERE user_id=?",
        ...
        if str(picture) != "<FileStorage: '' ('application/octet-stream')>":
            ...
            while os.path.exists(picture_path):
                hash_object = md5(
                    str(generate_random_number()).encode()).hexdigest()
                picture_path = "static/media/pictures/{}.png".format(
                    hash_object)
            picture.save(picture_path)
            db.execute("UPDATE users SET picture_path=? WHERE id=?",
...
def help():
    if request.method == "GET":
        ...
        subject = "[CliMan] Report from {} - {}".format(
